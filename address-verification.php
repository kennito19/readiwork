<?php
/**
 * Readiwork - Kenya Address Verification
 * Verify address information using national ID or passport
 */
    ...
    if (!preg_match('/^\d{7,8}$/', $id_number)) {
        ...
        try {
            ...
            if (!$yv_endpoint) {
                ...
                error_log("=== ADDRESS API FULL DEBUG ===");
                error_log("Endpoint Used: " . $yv_endpoint);
                error_log("Request Data: " . json_encode($yv_data, JSON_PRETTY_PRINT));
                error_log("HTTP Code: " . ($yv_result['http_code'] ?? 'NULL'));
                error_log("Success Flag: " . ($yv_result['success'] ? 'TRUE' : 'FALSE'));
                error_log("Full API Response: " . print_r($yv_result, true));
                // 🔍 END DEBUG
                ...
                if ($yv_result && $yv_result['success']) {
                    ...
                    if (!$reference_id) {
                        ...
                        $error = "No address records found for ID <strong>" . htmlspecialchars($id_number) . "</strong>.";
                        ...
                        try {
                            ...
                            $main_address = $api_data['mainAddress'] ?? null;
                            $secondary_address = $api_data['secondaryAddress'] ?? null;

                            // Store addresses as JSON in address field
                            $address_info = null;
                            if ($main_address || $secondary_address) {
                    ...
                    if ($http_code === 401) {
                        ...
                        $error = "No address records found for this ID.";
                        $errorIcon = 'user-slash';
                    } else {
...
<head>
<meta charset="utf-8">
<title>AI Kenya Address Verification — Readiwork AI</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="description" content="Verify address information for Kenyan citizens using official records.">
...
.form-select{background-image:url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3e%3cpath fill='none' stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='m2 5 6 6 6-6'/%3e%3c/svg%3e");padding-right:48px;}
...
<section class="page-hero">
  <div class="container">
    <h1><i class="fa-solid fa-robot"></i> Kenya <span>Address</span> Verification<span class="provider-badge provider-youverify">AI + YouVerify</span></h1>
    <p>Verify address information and residential details using official records.</p>
  </div>
</section>
        ...
        <div class="row g-5 align-items-stretch">
                ...
                <div class="form-card">
                    ...
                    <form method="POST" id="verificationForm">
                        ...
                        <div class="form-note">
                            ...
                            Your National ID is used strictly for address verification purposes.
                        </div>
            ...
            <div class="col-lg-5 order-2 order-lg-1">
                <div class="info-card">
                    ...
                    <ul class="info-list">
                        <li><i class="fa-solid fa-check"></i> Main residential address verification</li>
                        <li><i class="fa-solid fa-check"></i> Secondary address details</li>
                        <li><i class="fa-solid fa-check"></i> Town and postal code information</li>
