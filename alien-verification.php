<?php
/**
 * Readiwork - Kenya Alien ID Verification
 * Simple dedicated page - Alien ID ONLY
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['national_id'])) {
    $alien_id = trim($_POST['national_id']);

    // Validate Alien ID format: 6-10 digits
    if (!preg_match('/^\d{6,10}$/', $alien_id)) {
        $error = "Invalid Alien ID format. Must be 6-10 digits (e.g. 111111111).";
        $errorIcon = 'exclamation-circle';
    } else {
        try {
            $service_type = 'yv-ke-alien-id';
            ...
            if (!$yv_endpoint) {
                ...
                // Build YouVerify request - Alien ID only needs ID + consent
                $yv_data = [
                    'id' => $alien_id,
                ...
                if ($yv_result && $yv_result['success']) {
                    ...
                    if (!$reference_id) {
                        ...
                        $error = "Alien ID <strong>" . htmlspecialchars($alien_id) . "</strong> was not found in official records.";
                    ...
                    if ($http_code === 401) {
                        ...
                        $error = "Alien ID not found in YouVerify database.";
                        $errorIcon = 'user-slash';
                    } else {
...
<head>
<meta charset="utf-8">
<title>AI Kenya Alien ID Verification — Readiwork AI</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="description" content="Verify Kenya Alien ID authenticity and validity with official immigration records.">
...
<section class="page-hero">
  <div class="container">
    <h1><i class="fa-solid fa-robot"></i> Kenya <span>Alien ID</span> Verification<span class="provider-badge provider-youverify">AI + YouVerify</span></h1>
    <p>Verify Alien/Foreigner ID authenticity and validity with official Kenya immigration records.</p>
  </div>
</section>
    ...
    <div class="container">
        <div class="row g-5 align-items-stretch">
                ...
                <div class="form-card">
                    ...
                    <form method="POST" id="verificationForm">
                        <div class="mb-4 form-group">
                            <label class="form-label">Alien ID Number</label>
                            <i class="fa-solid fa-id-card"></i>
                            <input type="text" name="national_id" id="alienInput" class="form-control"
                                   value="<?= htmlspecialchars($alien_id) ?>"
                                   placeholder="e.g. 111111111" required maxlength="10" pattern="\d{6,10}">
                        </div>

                        <div class="form-note">
                            <i class="fa-solid fa-circle-info"></i>
                            Your Alien ID is used strictly for verification purposes with Kenya immigration records.
                        </div>
            ...
            <div class="col-lg-5 order-2 order-lg-1">
                <div class="info-card">
                    <h4>What this Alien ID check includes</h4>
                    <ul class="info-list">
                        <li><i class="fa-solid fa-check"></i> Alien ID number validation with official immigration database</li>
                        <li><i class="fa-solid fa-check"></i> Document authenticity & validity confirmation</li>
                        <li><i class="fa-solid fa-check"></i> Holder's full legal name</li>
                        <li><i class="fa-solid fa-check"></i> Date of birth and nationality</li>
                        <li><i class="fa-solid fa-check"></i> Gender information</li>
...
<script>
// Force numeric input only for Alien ID
document.getElementById('alienInput').addEventListener('input', function() {
