<?php
/**
 * Readiwork - Kenya Vehicle Collateral Verification
 * Simple dedicated page - collateral ID ONLY
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['national_id'])) {
    $collateral_id = trim($_POST['national_id']);

    // Validate format: 9 digits
    if (!preg_match('/^\d{9}$/', $collateral_id)) {
        $error = "Invalid collateral ID format. Must be 9 digits (e.g. 111111111).";
        $errorIcon = 'exclamation-circle';
    } else {
        try {
            $service_type = 'yv-ke-collateral';
            ...
            if (!$yv_endpoint) {
                ...
                // Build YouVerify request - collateral only needs ID + consent
                $yv_data = [
                    'id' => $collateral_id,
                ...
                if ($yv_result && $yv_result['success']) {
                    ...
                    if (!$reference_id) {
                        ...
                        $error = "Collateral ID <strong>" . htmlspecialchars($collateral_id) . "</strong> was not found in official records.";
                        $errorIcon = 'car-burst';
                    } else {
                        try {
                            // Extract collateral data from response
                            $collaterals = $api_data['collaterals'] ?? [];
                            $creditors = $api_data['creditors'] ?? [];
                            $currency_amounts = $api_data['currencyAmounts'] ?? [];
                            
                            // Serialize complex data for storage
                            $collateral_info = !empty($collaterals) ? json_encode($collaterals) : null;
                            $creditor_info = !empty($creditors) ? json_encode($creditors) : null;
                            $amount_info = !empty($currency_amounts) ? json_encode($currency_amounts) : null;
                            ...
                            $stmt->execute([
                                ...
                                ':dob'      => null,
                                ':phone'    => null,
                                ':bname'    => $creditor_info,  // Store creditor info in business_name field
                                ':addr'     => $collateral_info, // Store collateral info in address field
                    ...
                    if ($http_code === 401) {
                        ...
                        $error = "Collateral ID not found in YouVerify database.";
                        $errorIcon = 'car-burst';
                    } else {
...
<head>
<meta charset="utf-8">
<title>AI Kenya Vehicle Collateral Verification — Readiwork AI</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="description" content="Verify Kenya vehicle collateral registration and security interest details with official records.">
...
<section class="page-hero">
  <div class="container">
    <h1><i class="fa-solid fa-robot"></i> Kenya <span>Vehicle Collateral</span> Verification<span class="provider-badge provider-youverify">AI + YouVerify</span></h1>
    <p>Verify Kenya vehicle collateral registration and security interest details with official movable property registry.</p>
  </div>
</section>
    ...
    <div class="container">
        <div class="row g-5 align-items-stretch">
                ...
                <div class="form-card">
                    ...
                    <form method="POST" id="verificationForm">
                        <div class="mb-4 form-group">
                            <label class="form-label">Vehicle Collateral ID</label>
                            <i class="fa-solid fa-car"></i>
                            <input type="text" name="national_id" id="collateralInput" class="form-control"
                                   value="<?= htmlspecialchars($collateral_id) ?>"
                                   placeholder="e.g. 111111111" required maxlength="9" pattern="\d{9}">
                        </div>

                        <div class="form-note">
                            <i class="fa-solid fa-circle-info"></i>
                            Your collateral ID is used strictly for verification purposes. This service checks the Movable Property Security Rights Registry.
                        </div>
            ...
            <div class="col-lg-5 order-2 order-lg-1">
                <div class="info-card">
                    <h4>What this collateral check includes</h4>
                    <ul class="info-list">
                        <li><i class="fa-solid fa-check"></i> Vehicle collateral registration validation with official registry</li>
                        <li><i class="fa-solid fa-check"></i> Secured creditor information and details</li>
                        <li><i class="fa-solid fa-check"></i> Collateral description (make, model, registration)</li>
                        <li><i class="fa-solid fa-check"></i> Security interest amount and currency</li>
                        <li><i class="fa-solid fa-check"></i> Serial/chassis number verification</li>
                    ...
                    <div class="info-badges">
                        ...
                        <div class="info-badge"><i class="fa-solid fa-shield-halved"></i>Official</div>
...
<script>
// Force numeric input only for collateral ID
document.getElementById('collateralInput').addEventListener('input', function() {
